...
So, after putting this innital state in git, I'll put the gem `cucumber-rails` along with `database_cleaner` in my Gemfile under the test group and do a `bundle install` to get it included. The second of those gems does what it says on the tin and we'll also be using it with RSpec later.
